...
class SteamUnlockerApp(ctk.CTk):
    ...
    def __init__(self, cfg):
        ...
        self._cfg    = cfg
        self._games  = []
        self._game   = None    # currently displayed game dict
        self._data   = None    # analysis result for current game
        self._q      = _queue.Queue()
    ...
    def _build_ui(self):
        ...
        self._lbl_game_name = ctk.CTkLabel(hdr, text=G("lbl_select"),
                                            font=ctk.CTkFont(size=18, weight="bold"),
                                            anchor="w")
        self._lbl_game_name.grid(row=0, column=0, padx=14, pady=(10, 2), sticky="ew")
    ...
    def _populate_game_list(self, games):
        ...
        for g in games:
            ...
            lbl_name = ctk.CTkLabel(row, text=name[:38], anchor="w",
                                     font=ctk.CTkFont(size=12))
            lbl_name.grid(row=0, column=0, sticky="w", padx=8, pady=(4, 0))
            lbl_time = ctk.CTkLabel(row, text=playtime, anchor="w",
                                     text_color=self.C_SUB, font=ctk.CTkFont(size=10))
            lbl_time.grid(row=1, column=0, sticky="w", padx=8, pady=(0, 4))

            for w in (row, lbl_name, lbl_time):
    ...
    def _on_game_data(self, data):
        ...
        if not to_unlock and not to_remove:
            ...
            for a in to_unlock:
                ...
                display  = schema.get(api_name, {}).get("displayName", api_name)
                pct      = global_pct.get(api_name, 0.0)
                self._add_list_item(self._list_unlock, display, f"{pct:.1f}%", self.C_TEXT)

            for a in to_remove:
                api_name = a["apiname"]
                display  = schema.get(api_name, {}).get("displayName", api_name)
                pct      = global_pct.get(api_name, 0.0)
                self._add_list_item(self._list_remove, display, f"{pct:.1f}%", "#e67e22")
    ...
    def _add_list_item(self, parent, name, pct_text, color):
        row = ctk.CTkFrame(parent, fg_color="transparent")
        row.pack(fill="x", pady=1)
        row.columnconfigure(0, weight=1)
        ctk.CTkLabel(row, text=name[:50], anchor="w",
                     text_color=color, font=ctk.CTkFont(size=11)).grid(
            row=0, column=0, sticky="w", padx=6)
        if pct_text:
            ctk.CTkLabel(row, text=pct_text, anchor="e",
                         text_color=self.C_SUB, font=ctk.CTkFont(size=10),
                         width=44).grid(row=0, column=1, sticky="e", padx=(0, 6))
